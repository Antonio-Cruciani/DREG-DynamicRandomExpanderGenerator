...
def make_plots(ordered_data, output_file_path, number_of_bins,
    ...
    if xi_h_star > 0:
        ...
        data["adj_hill_scaling_alpha"] = np.round(1./xi_h_star, decimals = 3)
    if noise_flag:
    ...
    if xi_m_star > 0:
        ...
        axes[0,1].plot((x[-1]), [l*(float(xmin)/x[-1])**(alpha)],
                                   ...
                                   markeredgewidth = 3, markersize = 10)
        data["moment_scaling_alpha"] = np.round(1./xi_m_star, decimals = 3)
    if noise_flag:
    ...
    if xi_k_star > 0:
        ...
        axes[0,1].plot((x[-1]), [l*(float(xmin)/x[-1])**(alpha)],
                                   ...
                                   markeredgewidth = 3, markersize = 10)
        data["kernel_scaling_alpha"] = np.round(1./xi_k_star, decimals = 3)
    ...
    if bootstrap_flag:
        axes[1,0].scatter([k_h_arr[k_h_star-1]], [xi_h_arr[k_h_star-1]],
                       ...
                       label = r"$\widehat{\xi}^{Hill}="\
                           ...
                           +r"$")
        data["xi_hill"] = np.round([xi_h_arr[k_h_star-1]][0], decimals = 3)
    ...
    if bootstrap_flag:
        axes[1,1].scatter([k_h_arr[k_h_star-1]], [xi_h_arr[k_h_star-1]],
                       ...
                       label = r"$\widehat{\xi}^{Hill}="\
                           +str(np.round([xi_h_arr[k_h_star-1]][0], decimals = 3))\
                           +r"$")
        #data["smooth_hill"] = np.round([xi_h_arr[k_h_star-1]][0], decimals = 3)
    ...
    if bootstrap_flag:
        axes[2,0].scatter([k_m_arr[k_m_star-1]], [xi_m_arr[k_m_star-1]],
                       ...
                       label = r"$\widehat{\xi}^{Moments}="\
                           ...
                           +r"$")
        data["xi_moments"] = np.round([xi_m_arr[k_m_star-1]][0], decimals = 3)
    ...
    if bootstrap_flag:
        axes[2,0].scatter([k_k_arr[k_k1_star-1]], [xi_k_arr[k_k1_star-1]],
                       ...
                       label = r"$\widehat{\xi}^{Kernel}="\
                           ...
                           +r"$")
        data["xi_kernel"] = np.round([xi_k_arr[k_k1_star-1]][0], decimals = 3)
    ...
    if bootstrap_flag:
        axes[2,1].scatter([k_m_arr[k_m_star-1]], [xi_m_arr[k_m_star-1]],
                       ...
                       label = r"$\widehat{\xi}^{Moments}="\
                           +str(np.round([xi_m_arr[k_m_star-1]][0], decimals = 3))\
                           +r"$")
        #data["moments_(-1,1)"] = np.round([xi_m_arr[k_m_star-1]][0], decimals = 3)
    #plot kernel-type
    ...
    if bootstrap_flag:
        axes[2,1].scatter([k_k_arr[k_k1_star-1]], [xi_k_arr[k_k1_star-1]],
                       ...
                       label = r"$\widehat{\xi}^{Kernel}="\
                           +str(np.round([xi_k_arr[k_k1_star-1]][0], decimals = 3))\
                           +r"$")
        #data["kernel_(-1,1)"] = np.round([xi_k_arr[k_k1_star-1]][0], decimals = 3)
...
def main_2(inputFile,outputFile,delimit):
    #ignore warnings other than explicit ones
    warnings.filterwarnings("ignore")
    parser = argparse.ArgumentParser(description =
                                     "Script to compute tail index estimates\
    ...
    parser.add_argument("--delimiter",
                        ...
                        type = str, default = "whitespace")
    #args = parser.parse_args()
    args = parser.parse_args([inputFile,outputFile, "--delimiter",delimit])
    # check arguments for consistency
    if args.nbins <= 0:
        parser.error("Number of bins should be greater than 0.")
    if args.rsmooth < 2:
    ...
    print("Selected fraction of order statistics boundary for AMSE minimization: %0.4f"%eps_stop)
    print("========================")
    data = make_plots(ordered_data, args.output_file_path, number_of_bins,
               r_smooth, alpha, hsteps, bootstrap_flag, t_bootstrap,
               r_bootstrap, diagnostic_plots_flag, eps_stop, 
               args.theta1, args.theta2, verbose, noise_flag,
               p_noise, args.savedata)
...
if __name__ == '__main__':
    ...
    delimit = "tab"
    InPath = "/home/antoniocruciani/Desktop/Research/TemporalGraphs/TemporalDegree/TemporalGraphMining/temporal_graph_mining/temporal-graphs-mining/tail-estimation-master/temporal_weigted_directed/"
    Outpath = "/home/antoniocruciani/Desktop/Research/TemporalGraphs/TemporalDegree/TemporalGraphMining/temporal_graph_mining/temporal-graphs-mining/tail-estimation-master/output_new_algo/roads/"
    #degreeType = "average-degree/"
    #degrees = ['union-degree','2hop-degree-NO','2hop-degree-geometric-NO','2hop-degree-YY','2hop-degree-geometric-YY','2hop-degree-MM','2hop-degree-geometric-MM','2hop-degree-DD','2hop-degree-geometric-DD','2hop-degree-HH','2hop-degree-geometric-HH']
    #degrees = ['2hop-NO','2hop-geometric-NO','2hop-YY','2hop-geometric-YY','2hop-MM','2hop-geometric-MM','2hop-DD','2hop-geometric-DD','2hop-HH','2hop-geometric-HH']
    #degrees = ['2hop','2hop-geometric','union_IN','union_OUT','average_IN','average_OUT']
    degrees = ['union_IN','union_OUT','average_IN','average_OUT']
    #degrees = ['deg','2hop','G2hop']
    for deg in degrees:
        print("Analysing degree: ",deg)
        degreeType = deg+'/'
        path = InPath+degreeType
        outpath = Outpath + degreeType
        f = []
        data = {}
        ...
        for file in f[0]:
            ...
            inputFile = path + file
            outputFile = outpath + file[0:-4]+".pdf"
            T = main_2(inputFile,outputFile,delimit)
            data[file] = T
            print("Esalpsed time (file) ",time.time()-t1)

        t2 = time.time()
        print("Elapsed time (total):", t2-t1)
        with open(outpath+'summary.json', 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
        indexes = list(data.keys())
        dat= []
        for elem in indexes:
            dat.append(data[elem])
        df = pd.DataFrame(data = dat,index = indexes)
        df.to_csv(outpath+"summary.csv")
'''
